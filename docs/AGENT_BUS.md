# codexctl Agent Bus — Design Specification

**Status:** Draft / RFC. Phases 1–4 of §12 implemented behind the `bus` Cargo feature (see [Implementation status](#implementation-status) below). Delivery mechanism verified against the current Codex Hooks reference (June 2026).
**Scope:** Turn codexctl from a session monitor into a message bus that lets running Codex instances discover each other and coordinate, with agent-driven routing, persistent mailboxes, and centralized content/flow guardrails.
...
| 2. In-process bus MCP server, autostarted | **Partial** — runs as a stdio subprocess (`codexctl bus stdio`); the in-process autostart-with-TUI form (Unix-socket singleton) is not built yet. | `src/bus/mcp.rs`, `codex-plugin/.mcp.json` |
| 3. Provisioning (role binding + plugin install + hook capability probe) | **Shipped via `codexctl init`** (PR #283) — five-phase wizard owns bus role binding alongside budget, brain, hooks, and skill suggestions. Lifecycle verbs: `init`, `init --non-interactive`, `init --check`, `init --remove`, `init --reset`. Tracking: [#257](https://github.com/aleadag/codexctl/issues/257). | `src/init/` |
| 4. Mailbox + directed `publish` / `read_inbox` | **Shipped** | `src/bus/store.rs`, `src/bus/mcp.rs` |
| 5. `Stop` hook → `read_inbox` (Trigger A) + `/inbox` fallback (Trigger B) | **Shipped (Trigger A continue-in-turn + Trigger B)** | `src/bus/stop_hook.rs`, `codex-plugin/hooks/scripts/inbox-drain.sh`, `codex-plugin/commands/inbox.md` |
...
| 9. Managed-artifact lifecycle (update/migrate/drift) | **Partial** — `init --upgrade` re-syncs hooks + plugin files + DB migrations (#327); drift surfaces in `codexctl doctor` via the `plugin version` row. | `src/init/`, `src/doctor.rs` |
| 10. Supervisor + long-horizon persistence | **Shipped** via PRs #352–#356 — coord task tables, pure reconciler + actuator, bus-native assignment, verifier gates (`run`/`brain`/`agent`), resume protocol with tree-state drift detection, health-check transition triggers, `codexctl supervisor` CLI tree, v1 NDJSON event schema, Prometheus exporter. See [supervisor section](#10-supervisor--long-horizon-persistence) below. | `src/coord/{supervisor,actuator,verify,resume,events,exporter,supervisor_cli,tasks,session_policy,hook_events}.rs`, `src/ingest.rs` |
| 11. TUI bus view | **Not started** | — |

**Build / run:** `cargo build --release --features bus`. End-to-end smoke covered: role bind → cwd inference (macOS symlink resolution) → directed send → priority-ordered drain → leading-`/` neutralization → policy rejections. MCP handshake (`initialize` + `notifications/initialized` + `tools/list`) verified end-to-end. **Not yet exercised:** the plugin loaded inside Codex driving real cross-session traffic.

The dependency cost of `--features bus` is a 6.4 MB release binary (vs. the default 3.5 MB) and a current-thread Tokio runtime inside `codexctl bus stdio`. The default build is otherwise untouched.

---

## 1. Motivation

codexctl already knows every running Codex session (PID, cwd, status, session ID) and already knows how to inject input into a session via terminal control (`tmux send-keys`, Kitty/Ghostty remote control, etc.). Today these are exposed only as TUI keybinds, and the loop is never closed: codexctl *detects* when a session is `Waiting`, and can *inject* input, but never connects the two.

This spec closes that loop by exposing codexctl as an **MCP server**: a directory of running agents plus a persistent mailbox per agent. Running instances then coordinate themselves — the "what is my next step" decision lives in each agent's own reasoning and transcript, rather than in an external orchestrator.

### The MCP server starts with codexctl (no separate command)

The MCP server is **not** a separate `codexctl mcp` invocation the user must remember. When `codexctl` launches (TUI or any mode), it starts the bus MCP server **by default** as part of normal startup — same process, one core, the TUI and the MCP server are two frontends over it (§7). The server is up whenever codexctl is running, which is exactly when agents need it. Flags exist to disable it (`--no-bus`) or run headless server-only (`--bus-only`, for a daemon/CI context with no TUI), but the default is: start codexctl, the bus is live.

### Relationship to Codex sessions (the key positioning)

Codex sessions are strong execution units, but codexctl owns the durable coordination layer around them: persistent role binding, mailbox delivery, task state, file-claim tracking, and cross-session supervision. The bus should **not reinvent Codex execution**; it should make independent Codex sessions coordinate over shared local state.

Codex work is often bursty — create → parallelize one task → synthesize → tear down, on a minutes-to-hours horizon — while codexctl's differentiated role is the **durable supervision and persistence layer around those bursts**:

- Codex sessions are the *execution unit*.
- codexctl is the *long-horizon layer*: a durable role directory and mailbox that outlive any team's teardown, supervision/restart of dead leads, and orchestration of repeated bursts over a project that runs for weeks, months, or longer.
...
- **Agent bus (this spec):** codexctl is a bus the agents query and drive. Routing intelligence lives in the agents.

The bus is the better fit when steps are not a fixed graph (usually the case with Codex instances). `workflow.toml` remains available as an opt-in for genuinely fixed orchestration.
...
**Alignment with native Codex events.** Codex exposes `TaskCreated` and `TaskCompleted` lifecycle hooks (both can block on exit 2 — rolling back creation or preventing completion). Where agents use Codex's native task mechanism, the bus's `task.created` / `task.completed` subjects and the claim protocol should bind to these real events rather than being simulated, so task state on the bus stays consistent with the agent's own task state. This is an integration opportunity, not a requirement — bus subjects also work for agents not using native tasks.

---

## 3. MCP tool surface

Exposed by a `codexctl mcp` subcommand (see §7). All tools are namespaced under the codexctl MCP server each instance adds to its own config.
...
- codexctl delivers only on the recipient's `Processing → Waiting` edge (§6).
- Messages survive a recipient restart, because the mailbox is **codexctl's state**, not keystrokes typed into a now-dead pane.
...
**Persistence:** SQLite (WAL mode) under `~/.config/codexctl/bus.db`. Chosen over flat JSON because multiple session processes may touch the store concurrently; WAL gives safe concurrent writers and crash recovery. Schema sketch:
...
1. **Explicit `--role`** (CLI) / `CODEXCTL_BUS_ROLE` env at launch — used by `codexctl --new --role planner ...` and ambient scripting. Always wins when set.
...
| How | When | Mnemonic |
|---|---|---|
| `codexctl bus role bind <name> <cwd>` | provisioning a worktree, CI scripts | cwd-keyed (original) |
| `codexctl bus role bind <name> <cwd> --pid <pid>` | known pid (orchestrator path) | cwd + pid pinned |
| TUI **`Ctrl+R`** on the selected session | dashboard operator | binds selected session's pid + cwd |
| `/role <name>` (Codex slash command) | inside a running session | `--self` walks ancestry to find Codex's pid + uses current cwd |

**Don't have a role name in mind?** `codexctl bus role suggest --pid <pid>` (or `--self`-style auto-detection from a Codex session, or no `--pid` to walk the caller's ancestor chain) scans the session's transcript and cwd for signals — explicit "you are the X" mentions, tool fan-out shape (writes-heavy → `impl`, reads-heavy → `reviewer`, test-runs → `tester`), path patterns the session touches, and the cwd basename — and returns ranked candidates with reasons. Pure analysis: never writes a binding, never queries the LLM. Used by the TUI's Ctrl+R prompt and the `/role` slash command to pre-fill a sensible default (#309).
...
This is **push, not periodic polling**. The agent does nothing proactive; it is woken when it next finishes a turn and has mail. Codex's mature hook system (verified against the current Hooks reference) makes the primary path far cleaner than terminal injection — codexctl exposes `read_inbox` as an MCP tool, and the recipient's own `Stop` hook calls it. There are two triggers; Trigger A is now strongly preferred and Trigger B is a rarely-needed fallback.

### Trigger A — Codex `Stop` hook → codexctl MCP tool (primary)

Codex's `Stop` event fires when the model finishes responding, and a hook handler may be of type `mcp_tool`, calling a tool on an already-connected MCP server. So the recipient's `Stop` hook calls codexctl's `read_inbox` MCP tool **directly** — no terminal injection, no `/dev/tty`, no slash command needed for the common case. The agent drains its mailbox the instant it goes idle, through a fully controlled, structured path.
...
### Trigger B — codexctl `Waiting`-edge injection (fallback only)

Used only where a `Stop` hook can't be installed — e.g. an enterprise `allowManagedHooksOnly` policy blocks user/project/plugin hooks (§8), or the agent config is otherwise not under codexctl's control. codexctl falls back to what it already does: watch the JSONL, detect the `Processing → Waiting` edge, inject a short `/inbox` nudge (§9) via the terminal path. Same barge-in safety (delivery only at idle), but less clean and more terminal-dependent than Trigger A.

### Loop safety

The "two agents volley forever" failure mode is handled by codexctl's bus-level hop caps, loop guards, and mailbox dedupe (§10). Stop-hook delivery is still only a trigger mechanism; codexctl owns the durable safety policy.
...
The MCP server runs **in-process with codexctl and starts automatically on launch** (§1) — there is no separate start command. codexctl is one process exposing two frontends over a shared core: the TUI and the bus MCP server. The server listens on a local transport (Unix socket / stdio bridge as appropriate); agents connect to it via the MCP registration that `codexctl init` installs (§8).

Lifecycle:

- `codexctl` (normal) → TUI **and** bus server both up.
- `codexctl --bus-only` → headless server, no TUI (daemon / CI / long-running supervisor host).
- `codexctl --no-bus` → TUI only, bus disabled (pure monitoring, the legacy behavior).
...
A single shared in-process server serves all sessions (one shared mailbox store), rather than one server per session. Because the server's lifetime is codexctl's lifetime, the durable state (SQLite, §4) — not the process — is what guarantees messages and role addresses survive across codexctl restarts.

---

## 8. Agent-side provisioning & lifecycle

For the bus to work, three things must exist in each participating agent's Codex config: the **MCP server registration**, the **`Stop` hook** (Trigger A — now the primary path), and the **`/inbox` slash command** (Trigger B fallback + optional `/loop` use). Today codexctl provisions none of this — every user would hand-wire three things per worktree and get them subtly wrong. codexctl MUST own the full lifecycle of these artifacts: install, update, verify, and uninstall.

### Provisioning lives inside `codexctl init`, not a separate command

There is **no `codexctl setup` verb.** The single canonical entry point for getting an environment ready is [**`codexctl init`**](https://github.com/aleadag/codexctl/issues/257) — the same opinionated 60-second onboarding flow that handles the budget cap, brain auto-pilot detection, supervisor plugin install, and curated skill suggestions. Bus-readiness is one of its phases, not a parallel competing wizard. This matters because the worst onboarding bug we could ship is two separate "first-run" wizards a new user has to discover (the [dashingsauce r/CodexCode comment](https://github.com/aleadag/codexctl/issues/257) names that pattern explicitly: *"you end up connecting the same thing five times in five places"*).

`codexctl init` (no args) launches the interactive path; `--non-interactive` runs the same flow with prompted defaults for CI / dotfile automation. The bus-specific phases of that flow:
...
4. **Install the codexctl-bus plugin (or fallback writes)** — MCP server registration, `Stop` hook, `/inbox` command.
...
```
codexctl bus role bind implementer ~/proj-wt-impl
```

Future shape, once `init` lands:

```
codexctl init --non-interactive \
  --bus-role implementer --bus-cwd ~/proj-wt-impl \
  --bus-trigger stop-hook --bus-subscribe "task.*"
```

### Package as a Codex plugin (preferred mechanism)

Codex plugins bundle hooks (`hooks/hooks.json`), slash commands, and MCP server config in a single installable unit. codexctl ships **one "codexctl-bus" plugin** carrying all three artifacts together, rather than editing `.codex/hooks.json`, the commands dir, and the MCP config as three separate writes. Benefits: atomic install/uninstall, a single version to migrate, and plugin-owned paths give codexctl a clean home for hook scripts and persistent state. `codexctl init` enables/configures this plugin; uninstall removes the plugin cleanly with no residue in user config.

### Fallback: direct settings write

Where plugins are unavailable or an enterprise `allowManagedHooksOnly` policy blocks user/project/plugin hooks, `init` falls back to writing fenced entries into `.codex/hooks.json` (or detecting that hooks are blocked entirely, in which case it provisions only the MCP server + `/inbox` command and selects **Trigger B** for that role).

### Managed lifecycle (codexctl owns these, not the user)

codexctl tracks every artifact it installs in its own state (`~/.codexctl/`) and manages the full lifecycle:
...
- **verify / status** — `codexctl init --check` reports whether each artifact is present, current-version, drifted, or missing. Drift = artifact edited out-of-band or stale after a codexctl upgrade.
- **update / migrate** — on codexctl upgrade, re-render artifacts to the new version; `init` offers to migrate detected stale installs.
- **uninstall** — `codexctl init --remove [--role X]` cleanly removes only codexctl-managed artifacts, leaving unmanaged config intact.
...
- Managed artifacts are **tagged/fenced** (e.g. a `# >>> codexctl managed >>>` marker block, or a dedicated managed file the user's config imports) so codexctl can update or remove exactly its own content without touching hand-written config.
- All operations are **idempotent** — re-running `init` converges to the desired state, never duplicates.
- codexctl **never silently overwrites** unmanaged content; conflicts surface in the interactive flow or fail loudly in `--non-interactive` mode.
- A marker file `~/.codexctl/onboarding.json` records the last completed init version so the flow doesn't re-prompt unnecessarily.

This makes "is my environment bus-ready and current?" a single `codexctl init --check`, and makes onboarding a new worktree a single `init` pass rather than three manual edits.

---

## 9. Slash commands: risk and capability

Codex sessions interpret slash commands (`/loop`, custom commands). This cuts both ways and shapes two requirements.
...
Conversely, a custom slash command is a *better* nudge than raw text. Define `/inbox` (or similar) in each agent's config; codexctl's nudge is simply "run `/inbox`." The agent pulls its mailbox through a path it explicitly controls, sidestepping barge-in — a slash command is unambiguous and self-contained in a way a pasted paragraph is not.

### Resolution

Commands flow **one direction only**: codexctl → session, as the controlled wake signal (`/inbox`). Message bodies flowing the other direction are stripped of all command syntax. This separates the control channel from the content channel cleanly.

---

## 10. Guardrails

codexctl is the single process every message passes through, making it the correct, unbypassable enforcement point. Guardrails are **policy, not hardcoded** — a `[bus.policy]` config block with strict defaults, loosened opt-in.
...
- **Managed-artifact fencing mechanism** — codexctl-bus plugin (preferred, atomic) vs. fenced marker-block in `.codex/hooks.json` (fallback). Plugin assumed primary; confirm.
...
2. **In-process bus MCP server, autostarted with codexctl** — server skeleton over existing discovery; up by default on launch, `--no-bus` / `--bus-only` flags. No separate start command.
3. **Provisioning via `codexctl init` (§8, tracked in #257)** — bus-readiness phases (role assignment + managed install of MCP registration and `/inbox` command, with `--check`/`--remove`) fold into the umbrella `codexctl init` flow rather than a separate `setup` verb. Needed early: every later step assumes agents are wired up, and manual wiring is the main onboarding friction.
...
The target use case — agents collaborating continuously over a project that runs for weeks, months, or longer — is **not** served by native agent teams, which are a bounded-burst tool. The verified limitations that make teams unfit for long-horizon operation, and what codexctl supplies instead:

| Agent-teams limitation (native) | Consequence for long-running work | codexctl supervisor provides |
| --- | --- | --- |
...
1. A **durable supervisor** (codexctl `--bus-only` or the running TUI) holds the long-lived state: the role directory, the persistent mailboxes, the project-level task backlog, subscriptions, and policy. This survives indefinitely because it is process-independent SQLite state, not a set of live sessions.
...
This gives the months/years horizon as a *supervised sequence of cheap-to-persist, expensive-only-when-active episodes*, rather than an impossible continuous team. Native teams are the muscle; codexctl is the skeleton and memory that outlive any single contraction.

### What must be durable (survives process death, codexctl restart, team teardown)

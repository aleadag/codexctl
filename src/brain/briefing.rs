...
//!
//! Every new Codex session starts cold. The brain already accumulates
...
pub struct BriefingOptions {
    ...
    pub include_agents_md_check: bool,
}
...
pub fn build_briefing(opts: &BriefingOptions, cwd: &Path) -> String {
    ...
    sections.push(
        "_Auto-generated by `coding-brain --brain-briefing`. Reflects state of the brain at session start._"
            .to_string(),
    );
    ...
    if opts.include_agents_md_check {
        if let Some(path) = find_agents_md(cwd) {
            sections.push(format!(
                "## Project guidance\n\n- `AGENTS.md` is loaded automatically by Codex: `{}`",
                path.display()
            ));
        }
    }
...
mod tests {
    ...
    fn briefing_is_self_explanatory_when_empty() {
        ...
        // machine's real Coding Brain state (which may have decisions/preferences).

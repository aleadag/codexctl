...
use codexctl_core::session::{CodexSession, SessionStatus, SubagentBreakdown, SubagentState};
...
pub fn render(frame: &mut Frame, area: Rect, app: &App) {
    ...
    if app.sessions.is_empty() {
        let launch_hint = if codexctl_core::terminals::can_launch_session() {
            "  Press n for the launch wizard, or start codex in another terminal."
        } else {
            "  Start codex in GNOME Terminal, tmux, Kitty, WezTerm, Windows Terminal on WSL, or another terminal."
        };
        let empty_lines = vec![
            Line::from(""),
            Line::from(""),
            ...
            Line::from("  Run codexctl doctor if terminal switching or launch fails."),
            Line::from(""),
            Line::from(vec![
...
fn render_rows_for_session(s: &CodexSession, app: &App) -> Vec<Row<'static>> {
...
fn session_row(s: &CodexSession, app: &App) -> Row<'static> {
    let t = &app.theme;

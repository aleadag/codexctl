...
pub struct App {
    ...
    pub theme: Theme,
    pub weekly_summary: codexctl_core::history::WeeklySummary,
    ...
    pub rules: Vec<codexctl_core::rules::AutoRule>,
    ...
    pub health_thresholds: codexctl_core::health::HealthThresholds,
    pub brain_config: Option<codexctl_core::config::BrainConfig>,
    ...
    pub brain_queue: Vec<codexctl_core::runtime::ReviewItemSummary>,
    /// All decision records loaded for the scorecard view.
    pub brain_decisions_cache: Vec<codexctl_core::runtime::DecisionSummary>,
    /// Transient status message shown in the overlay footer.
...
/// Project a live `CodexSession` to the core `SessionSnapshot` DTO the
/// runtime traits accept. Used by `BrainDriver` call sites that have the
/// live values in memory already.
fn snapshot_from(session: &CodexSession) -> codexctl_core::runtime::SessionSnapshot {
    codexctl_core::runtime::SessionSnapshot {
...
impl App {
    pub fn new() -> Self {
        let mut app = Self {
            ...
            theme: Theme::from_mode(codexctl_core::theme::ThemeMode::Dark),
            weekly_summary: codexctl_core::history::weekly_summary(),
            ...
            health_thresholds: codexctl_core::health::HealthThresholds::default(),
            brain_config: None,
            brain_driver: None,
    ...
    pub fn refresh(&mut self) {
        ...
        let mut existing: HashMap<u32, CodexSession> =
        ...
        for session in &mut sessions {
            ...
            session.decay_score =
                codexctl_core::health::compute_decay_score(session, &self.health_thresholds);
        }
        ...
        for session in &sessions {
                ...
                && !self.finished_at.contains_key(&session.pid)
            {
                ...
                codexctl_core::history::record_session(session);
            }
        }
        sessions.retain(|s| {
    ...
    fn apply_sort(&self, sessions: &mut [CodexSession]) {
    ...
    fn run_auto_actions(&mut self) {
                ...
                if let Some(session) = self.sessions.iter().find(|s| s.pid == pid) {
                    ...
                    match self.runtime.actions.inject_text(&session.session_id, &msg) {
                        Ok(()) => {
                            ...
                            codexctl_core::logger::log("CONFLICT", &status);
        ...
        if !self.rules.is_empty() {
            ...
            for pid in candidates {
                ...
                let result = codexctl_core::rules::evaluate(&self.rules, session);
                ...
                let msg = codexctl_core::rules::execute(&rule_match, session);
                match msg {
                    Ok(status) => {
                        codexctl_core::logger::log("AUTO", &status);
        ...
        if let Some(ref mut driver) = self.brain_driver {
            ...
            let deny_rules: Vec<_> = self
                .rules
                .iter()
                .filter(|r| r.action == codexctl_core::rules::RuleAction::Deny)
                .cloned()
                .collect();
    ...
    pub fn selected_session(&self) -> Option<&CodexSession> {
    ...
    fn handle_brain_review_tab_key(&mut self, key: KeyEvent) {
        ...
        match key.code {
            KeyCode::Char('j') | KeyCode::Down if self.brain_review_selected < last => {
                self.brain_review_selected += 1;
            }
            KeyCode::Char('k') | KeyCode::Up if self.brain_review_selected > 0 => {
                self.brain_review_selected -= 1;
            }
    ...
    pub fn visible_sessions(&self) -> Vec<&CodexSession> {
    ...
    fn matches_search_query(&self, session: &CodexSession) -> bool {
    ...
    fn handle_brain_accept_with_reason(&mut self, override_reason: Option<&str>) {
        ...
        if let Some(msg) = driver.accept(pid) {
            ...
            codexctl_core::logger::log("BRAIN", &format!("Accepted: {msg}"));
            self.status_msg = msg;
        }
        self.pending_override_reason = None;
    }
    ...
    fn toggle_brain_gate(&mut self) {
        ...
        codexctl_core::logger::log("BRAIN", &format!("Gate mode toggled: {current} → {next}"));
    }
...
impl App {
    pub fn project_groups(&self) -> Vec<ProjectGroup> {
        let mut groups: HashMap<String, Vec<&CodexSession>> = HashMap::new();
...
mod tests {
    ...
    fn invalid_launch_keeps_wizard_open_and_reports_error() {
        let mut app = App::new();
        app.enter_launch_mode();
        app.launch_form.cwd = "/tmp/codexctl-this-path-should-not-exist".into();

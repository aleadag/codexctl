// Skill discovery: scans the standard Codex skill locations and returns
// metadata parsed from each skill's YAML frontmatter.
//
// Skills can live in three shapes:
//   1. ~/.codex/skills/<name>/SKILL.md            (user-installed)
//   2. ~/.codex/plugins/<plugin>/skills/<name>/   (shipped by a plugin)
//   3. <cwd>/.codex/skills/<name>/SKILL.md        (project-local)
//
// We also accept the flat form ~/.codex/skills/<name>.md for compatibility
...
fn scan_skill_root(
    ...
    for entry in entries.flatten() {
        ...
        if path.is_dir() {
            ...
            // Flat form: ~/.codex/skills/<name>.md

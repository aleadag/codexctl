...
use codexctl_core::session::{CodexSession, RawSession, SessionStatus, TelemetryStatus, ToolStats};
...
pub fn generate_sessions(tick: u32) -> Vec<CodexSession> {
    ...
    PROJECTS
        ...
        .map(|(i, (name, cwd, model))| {
            ...
            s.context_max = codexctl_core::monitor::model_context_max(model);
...
pub fn demo_rules() -> Vec<codexctl_core::rules::AutoRule> {
    use codexctl_core::rules::{AutoRule, RuleAction};
...
/// A scripted JSONL transcript simulating a realistic Codex session.
...
impl DemoHighlightState {
    ...
    pub fn ensure_jsonl(&mut self, pid: u32) -> &PathBuf {
        let entry = self.sessions.entry(pid).or_insert_with(|| {
            ...
            path.push(format!("codexctl-demo-{pid}.jsonl"));
...
mod highlight_tests {
    use super::*;
    use codexctl_core::transcript::{TranscriptEvent, parse_line};
    ...
    fn demo_highlight_generates_valid_jsonl() {
        ...
        for (i, line) in script.iter().enumerate() {
            ...
            if let TranscriptEvent::Message(msg) = event {
                match msg.role {
                    codexctl_core::transcript::TranscriptRole::Assistant => assistant_count += 1,
                    codexctl_core::transcript::TranscriptRole::User => {
                        for block in &msg.content {
                            if let codexctl_core::transcript::TranscriptBlock::ToolResult {
                                is_error,
                                ..
                            } = block
                            {

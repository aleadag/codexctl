...
// session table. Two tabs: Skills (discovered Codex skills + share)
// and Hive (identity, peers, invite, join, start listener). All hive-side
// actions resolve to `codexctl relay …` subprocesses so the TUI event loop
// stays responsive.
...
fn render_skills_body(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let detail = if let Some(s) = app.skills.get(app.skills_selected) {
        let shared = codexctl_core::skills::is_shared(s, &app.shared_skill_keys);
...
fn skill_line<'a>(skill: &'a DiscoveredSkill, app: &'a App, t: &'a Theme) -> Line<'a> {
    let shared = codexctl_core::skills::is_shared(skill, &app.shared_skill_keys);
...
mod tests {
    ...
    fn skill_line_renders_shared_marker() {
        ...
        let skill = DiscoveredSkill {
            ...
            source: codexctl_core::skills::SkillSource::User,
            plugin: None,
            size_bytes: 100,
        };
